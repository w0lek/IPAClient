...
class TelegramHeader {
 public:
  ...
  explicit TelegramHeader(const QByteArray& buffer);
  ~TelegramHeader() = default;

  static comm::TelegramHeader constructFromBody(const QByteArray& body,
                                                uint8_t compressorId = 0) {
      uint32_t bodyCheckSum = calcCheckSum(body);
